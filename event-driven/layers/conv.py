...
class ConvFunc(torch.autograd.Function):
    ...
    def backward(ctx, grad_delta):
        ...
        # 使用 PyTorch 内置函数替代 CUDNN 函数
        # 对输入的梯度：使用转置卷积
        grad_input_transposed = f.conv_transpose2d(
            grad_in_.to(weight_), weight_, None, stride, padding, 
            groups=groups, dilation=dilation
        )
        
        # 确保转置卷积的输出形状与 inputs 匹配
        if grad_input_transposed.shape != inputs.shape:
            _, C_in, H_in, W_in = inputs.shape
            _, C_grad, H_grad, W_grad = grad_input_transposed.shape
            ...
            if H_grad > H_in:
                ...
                pad_h = H_in - H_grad
                grad_input_transposed = f.pad(grad_input_transposed, (0, 0, 0, pad_h), mode='constant', value=0)
            
            if W_grad > W_in:
                grad_input_transposed = grad_input_transposed[:, :, :, :W_in]
            elif W_grad < W_in:
                pad_w = W_in - W_grad
                grad_input_transposed = f.pad(grad_input_transposed, (0, pad_w, 0, 0), mode='constant', value=0)
        
        grad_input = grad_input_transposed * inputs
        
        # 计算权重梯度：使用 unfold 操作
        # grad_w_ 形状: (T*n_batch, C_out, H_out, W_out)
        # inputs 形状: (T*n_batch, C_in, H_in, W_in)
        out_channels, in_channels, kernel_h, kernel_w = weight.shape
        
        # 使用 unfold 来提取输入的 patch
        inputs_unfolded = f.unfold(inputs, (kernel_h, kernel_w), stride=stride, padding=padding, dilation=dilation)
        # inputs_unfolded: (T*n_batch, C_in * kernel_h * kernel_w, out_h * out_w)
        
        grad_weight = torch.zeros_like(weight)
        
        # 对每个输出通道计算梯度
        for c_out in range(out_channels):
            grad_w_c = grad_w_[:, c_out, :, :]  # (T*n_batch, H_out, W_out)
            grad_w_c = grad_w_c.contiguous().view(T * n_batch, 1, -1)  # (T*n_batch, 1, out_h * out_w)
            
            # 计算权重梯度：grad_weight[c_out] = sum(inputs_unfolded * grad_w_c)
